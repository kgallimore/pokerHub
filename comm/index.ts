...
await pb.collection("CommPorts").subscribe<CommPortsResponse>("*", async (data) => {
  switch (data.action) {
    case "create":
    ...
    case "update":
      ...
      break;
    case "delete":
      commPorts.splice(commPorts.findIndex((port) => port.id === data.record.id), 1);
      break;
  }
});
const serialPorts: {
    ...
    for (const port of ports) {
      ...
      if (
        ...
        newPort.on("close", async () => {
          await postCommPortUpdate({ commPort: parseInt(portNum), connected: false });
...
function parseMessage(message: string, commPort: number) {
  ...
  if(Array.isArray(cardIds) === false){ return;}
  if (
...
async function postSensorUpdate(data: { commPort: number, sensor: number, value: Array<string> }) {
  if (serialPorts[data.commPort.toString()].numOfSensors === 0) return;
  const existingCommPort = commPorts.find((port) => port.port === data.commPort);
  if (!existingCommPort) {console.warn("Tried to post sensor update to null port");return};
  ...
  await pb.collection("Sensors").update(existingCommPort.sensors[data.sensor], {cards: cardIds});
...
async function postCommPortUpdate(data: {
  ...
  const existingCommPort = commPorts.find((port) => port.port === data.commPort);
  if(existingCommPort){
    if(data.numberOfSensors && existingCommPort.sensors.length !== data.numberOfSensors){
      const createdSensors: Array<string> = [];
        for(let i = existingCommPort.sensors.length; i < data.numberOfSensors; i++){
          createdSensors.push((await pb.collection("Sensors").create({
